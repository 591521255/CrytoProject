...
//            Scanner sc = new Scanner(System.in);
            ...
            while (isRunning) {
                ...
                for (int i = 0; i < len; i++) {
                    ...
                    if (Pattern.matches("[a-zA-Z0-9]*:[0-9]*:[0-9]*", tempString)) {
                        String[] split = temp[0].split(":");
                        switch (split[2]) {
                            case "1":
                                Double money = Double.valueOf(FondUtil.p.getProperty(split[0]));
                                FondUtil.p.setProperty(split[0], String.valueOf(money + Double.valueOf(split[1])));
                                FondUtil.store();
                                sendPacket("ok", dout);
                                break;
                            case "2":
                                money = Double.valueOf(FondUtil.p.getProperty(split[0]));
                                FondUtil.p.setProperty(split[0], String.valueOf(money - Double.valueOf(split[1])));
                                FondUtil.store();
                                sendPacket("ok", dout);
                                break;
                            case "3":
                                money = Double.valueOf(FondUtil.p.getProperty(split[0]));
                                sendPacket(String.valueOf(money), dout);
                                break;
                        }
